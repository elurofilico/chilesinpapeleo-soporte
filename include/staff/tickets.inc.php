...
$sortOptions=array('date'=>'ticket.created','ID'=>'ticketID','pri'=>'priority_urgency','dept'=>'dept_name','servicio'=>'servicio.nombre','topic'=>'topic.topic');
...
if(!$thisuser->isadmin()){ 
	$qwhere .= ' AND ticket.dept_id = '.$thisuser->udata['dept_id'];
}
...
$qselect.=' ,count(attach.attach_id) as attachments, IF(ticket.reopened is NULL,ticket.created,ticket.reopened) as effective_date, servicio.nombre as nombre_servicio, topic.topic as nombre_topic';
$qfrom.=' LEFT JOIN '.TICKET_PRIORITY_TABLE.' pri ON ticket.priority_id=pri.priority_id '.
        ' LEFT JOIN '.TICKET_LOCK_TABLE.' tlock ON ticket.ticket_id=tlock.ticket_id AND tlock.expire>NOW() '.
        ' LEFT JOIN '.TICKET_ATTACHMENT_TABLE.' attach ON  ticket.ticket_id=attach.ticket_id '.
        ' LEFT JOIN seg_servicio servicio ON  ticket.servicio_codigo=servicio.codigo ' .
 ...
 <input type="hidden" name="a" value="search">
  <table>
    <tr>
        ...
        <td>Asesor:</td>
        <td><select name="dept"><option value=0>Todos los asesores</option>
            <?php 
        ...
        <?php } ?>
        <td>Estado:</td>
        <td <?php echo !$thisuser->isadmin()?'colspan="2"':''; ?>>    
        <select name="status">
    ...
    <tr>
       ...
       <td>Ordenar por:</td><td>
        ...
        <select name="sort">
            ...
            <option value="dept" <?php echo  $sort == 'dept' ?'selected':''?>>Asesor</option>
        </select>
        <select name="order">
    ...
    <tr><td>
       <table width="100%" border="0" cellspacing=0 cellpadding=2 class="dtable" align="center">
        <tr>
	        ...
	        <th width="180">Asunto</th>
	        <th width="120">
                <a href="tickets.php?sort=dept&order=<?php echo $negorder?><?php echo $qstr?>" title="Ordenar por Asesor <?php echo $negorder?>">Asesor</a></th>
	        <th width="70">
            ...
            <th width="100" >De</th>
            <th width="180">
            	<a href="tickets.php?sort=servicio&order=<?php echo $negorder?><?php echo $qstr?>" title="Ordenar por Servicio <?php echo $negorder?>">Área de ayuda</a>
            </th>
            <th width="180">
                <a href="tickets.php?sort=topic&order=<?php echo $negorder?><?php echo $qstr?>" title="Ordenar por Servicio <?php echo $negorder?>">Institución</a>
            </th>
        </tr>
        <?php 
        $class = "row1";
        ...
        if($tickets_res && ($num=db_num_rows($tickets_res))):
            ...
            <tr class="<?php echo $class?> " id="<?php echo $row['ticket_id']?>">
                ...
                <td><?php echo $row['nombre_servicio']?$row['nombre_servicio']:'Sin servicio'; ?></td>
                <td><?php echo $row['nombre_topic']; ?></td>
            </tr>
            <?php 
