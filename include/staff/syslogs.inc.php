...
if( ($startTime && $startTime>time()) or ($startTime>$endTime && $endTime>0)){
    $errors['err']='Intervalo de tiempo invalido. La selecci&oacute;n sera ignorada.';
    $startTime=$endTime=0;    
}else{
...
?>
<div class="msg">Registros del Sistema</div>
<div id='filter' >
 <form action="admin.php?t=syslog" method="get">
    <input type="hidden" name="t" value="syslog" />
    <div style="padding-left:15px;">
        Buscar fecha:
        &nbsp;del&nbsp;<input id="sd" size=15 name="startDate" value="<?php echo Format::htmlchars($_REQUEST['startDate'])?>" 
            ...
            <select name='type'>
                ...
                <option value="Error" <?php echo ($type=='Error')?'selected="selected"':''?>>Errores</option>
                <option value="Warning" <?php echo ($type=='Warning')?'selected="selected"':''?>>Advertencias</option>
                <option value="Debug" <?php echo ($type=='Debug')?'selected="selected"':''?>>Depurados</option>
            </select>
            &nbsp;&nbsp;
...
<div style="margin-bottom:20px; padding-top:0px;">
 <table width="100%" border="0" cellspacing=1 cellpadding=2>
        ...
        if($result && ($num=db_num_rows($result))):
            ...
            <tr class="<?php echo $class?> " id="<?php echo $row['log_id']?>">
                <td>
                  ...
                  <span style="color:000; float: left; width:190px;"><?php echo Format::fecha($row['created'])?></span>
                  &nbsp;&nbsp;
            ...
            <tr class="<?php echo $class?>"><td><b>La consulta a devuelto 0 resultados.</b></td></tr>
        <?php 
        endif; ?>
       </table>
    </td></tr>
    ...
    if($num>0){ 
    ?>
        <tr><td style="padding-left:20px">P&aacute;gina:<?php echo $pageNav->getPageLinks()?></td></tr>
    <?php } ?>
    </form>
 </table>
</div>
