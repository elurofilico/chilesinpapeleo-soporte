...
class Validator {
    ...
    function validate($source,$userinput=true){
        ...
        if(!$source || !is_array($source))
            $this->errors['err']='Entrada no v&aacute;lida';
        elseif(!$this->fields || !is_array($this->fields))
            $this->errors['err']='No hay campos creados';
        //Abort on error
        if($this->errors)
            return false;
        ...
        foreach($this->fields as $k=>$field){
            ...
            case 'password':
                if(strlen($this->input[$k])<5)
                    $this->errors[$k]=$field['error'].' (5 caracteres m&iacute;nimo.)';
                break;
            case 'username':
                if(strlen($this->input[$k])<3)
                    $this->errors[$k]=$field['error'].' (3 caracteres m&iacute;nimo.)';
                break;
            case 'zipcode':
            ...
            default://If param type is not set...or handle..error out...
                $this->errors[$k]=$field['error'].' (No se a establecido tipo)';
            endswitch;
        }
    ...
    function is_phone($phone) {
        ...
        return (!is_numeric($stripped) || ((strlen($stripped)<3) || (strlen($stripped)>16)))?false:true;
    }
