...
class Topic {
    ...
    function save($id,$vars,&$errors) {
        ...
        if($id && $id!=$vars['topic_id'])
            $errors['err']='Error interno, intentalo de nuevo';

        if(!$vars['topic'])
            $errors['topic']='&Aacute;rea de ayuda requerida';
        elseif(strlen($vars['topic'])<5)
            $errors['topic']='El &aacute;rea de ayuda debe tener al menos 5 caracteres';
        else{
            $sql='SELECT topic_id FROM '.TOPIC_TABLE.' WHERE topic='.db_input(Format::striptags($vars['topic']));
            if($id)
                $sql.=' AND topic_id!='.db_input($id);
            if(($res=db_query($sql)) && db_num_rows($res))
                $errors['topic']='Esta &aacute;rea de ayuda ya existe';
        }
            
        if(!$vars['dept_id'])
            $errors['dept_id']='Debe seleccionar un departamento';
        ...
        if(!$errors) {
            ...
            if($id) {
                ...
                if(!db_query($sql) || !db_affected_rows())
                    $errors['err']='No se a podido actualizar el &aacute;rea de ayuda. Error interno'; 
            }else{
